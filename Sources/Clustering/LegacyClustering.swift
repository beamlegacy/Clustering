        ...
        didSet {
             if !isClustering {
                 for page in skippedPages {
                     self.add(textualItem: page.toTextualItem(), ranking: nil) { _ in }
                 }
                 self.skippedPages = []
                 for note in skippedNotes {
                     self.add(textualItem: note.toTextualItem(), ranking: nil) { _ in }
                 }
                 self.skippedNotes = []
             }
        }
...
//            self.adjacencyMatrix = textSigmoidMatrix .* navigationSigmoidMatrix + entitySigmoidMatrix
    ...
    public func removeTextualItem(textualItemUUID: UUID) {
        self.additionsInQueue += 1
        additionQueue.async {
            if let noteIndex = self.findNoteInNotes(noteID: textualItemUUID) {
           ...
           let id_index = self.findPageInPages(pageID: page.id) {
            ...
            do {
                if let newContent = note.originalContent {
                    (self.notes[id_index].cleanedContent, self.notes[id_index].language) = try self.extractor.extract(from: newContent, forType: DataPoint.note)
                }
            ...
            if let page = page {
                ...
                do {
                    (self.notes[newIndex].cleanedContent, self.notes[newIndex].language) = try self.extractor.extract(from: self.notes[newIndex].originalContent ?? [""], forType: DataPoint.note)
    ...
    public func add(textualItem: TextualItem, ranking: [UUID]?, activeSources: [UUID]? = nil, replaceContent: Bool = false, completion: @escaping (Result<(pageGroups: [[UUID]], noteGroups: [[UUID]], flag: Flag, similarities: [UUID: [UUID: Float]]), Error>) -> Void) {
        ...
        additionQueue.async {
            var page: Page? = nil
            var note: ClusteringNote? = nil
            
            if textualItem.type == TextualItemType.page {
                page = textualItem.toPage()
            } else {
                note = textualItem.toNote()
            }
