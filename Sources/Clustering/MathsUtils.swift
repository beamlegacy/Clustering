...
class MathsUtils {
    ...
    static func cosineSimilarity(vector1: [Double], vector2: [Double]) -> Double {
        ...
        let zeros = [Double](repeating: 0.0, count: vector1.count)
        
        if vector1Norm == zeros || vector2Norm == zeros {
        ...
        let similarity = vector1NormVector2NormDotProduct / (cblas_dnrm2(Int32(vector1Norm.count), vector1Norm, 1) * cblas_dnrm2(Int32(vector2Norm.count), vector2Norm, 1))
        //fputs("Swift: " + String(similarity.rounded(toPlaces: 4)) + "\n", stderr)
        let diff = CFAbsoluteTimeGetCurrent() - start
        print("Swift: execution time cosineSimilarity: \(diff * 1000) ms\n")
